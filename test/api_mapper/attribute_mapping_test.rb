...
module ApiMapper
  class AttributeMappingTest < Minitest::Test
    ...
    def test_mapping_from_array
      ...
      assert_equal("123", mapping.extract(MappingOrigin.new("id", "123")))
    end

    def test_nested_extract
      mapping = ApiMapper::AttributeMapping.new("user.id.name")

      assert_equal("john", mapping.extract(MappingOrigin.new("user", { "id" => { "name" => "john" } })))
    end

    def test_nested_extract_with_change
      mapping = ApiMapper::AttributeMapping.new("user.id.firstName", "name")

      assert_equal("john", mapping.extract(MappingOrigin.new("user", { "id" => { "firstName" => "john" } })))
    end

    def test_nested_extract_nil
      mapping = ApiMapper::AttributeMapping.new("user.age", "age")

      assert_raises(ApiMapper::AttributeMapping::Error) { mapping.extract(MappingOrigin.new("user", { "name" => "john" })) }
    end
